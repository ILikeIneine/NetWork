...
public:
    server_interface(uint16_t port) : acceptor_(ioc_, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), port))
    {
    }
        ...
        acceptor_.async_accept([this](std::error_code ec, asio::ip::tcp::socket socket)
                               {
                                   if (!ec)
                                   {
                                       ...
                                       std::shared_ptr<connection<T>> new_client = std::make_shared<connection<T>>(
                                               connection<T>::owner::server, ioc_, std::move(socket), qmsg_in_);

                                       if (on_client_connected(new_client))
                                       {
                                           // only a server instance, won't data racing
                                           this->qconns_.push_back(new_client);
                                           this->qconns_.back()->connect_to_client(this, ID_counter_++);
                                           std::cout << "[" << this->qconns_.back()->get_id()
                                                     << "] Connection Approved\n";
                                       } else
                                       {
    ...
    void message_client(std::shared_ptr<connection<T>> client, const message<T> msg)
    {
        ...
        } else
        {
            ...
            client.reset();
            qconns_.erase(std::remove(qconns_.begin(), qconns_.end(), client), qconns_.end());
        }
    }

    void message_all_client(const message<T> msg, std::shared_ptr<connection<T>> ignore_client = nullptr)
    {
        ...
        {
            qconns_.erase(std::remove(qconns_.begin(), qconns_.end(), nullptr), qconns_.end());
        }
    }
