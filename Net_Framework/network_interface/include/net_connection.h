...
public:
  connection(owner belonger, asio::io_context& ioc,
      asio::ip::tcp::socket socket, tsqueue<owned_message<T>>& q_in)
    : socket_(std::move(socket)), ioc_(ioc), qmsg_in_(q_in)
  {
    ...
    {
      handshake_out_ = static_cast<uint64_t>(std::chrono::system_clock::now().
...
private:
  void write_header()
  {
    asio::async_write(socket_,
        asio::buffer(&qmsg_out_.front().header, sizeof msg_hdr<T>),
  ...
  {
    asio::async_write(socket_,
        asio::buffer(qmsg_out_.front().body.data(), qmsg_out_.front().size()),
        [this](std::error_code ec, std::size_t length)
          {
            if (!ec)
            {
  ...
  void read_header()
  {
    ...
    asio::async_read(socket_,
        asio::buffer(&msg_temporary_in_.header, sizeof msg_hdr<T>),
        [this](std::error_code ec, std::size_t length)
          {
            if (!ec)
            {
              ...
              {
                this->msg_temporary_in_.body.resize(
  ...
  {
    asio::async_read(socket_,
        asio::buffer(&handshake_in_, sizeof handshake_in_),
